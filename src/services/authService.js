import bcrypt from 'bcryptjs';
  ...
  export const register = async ({ name, email, mobile, password }) => {
    ...
    try {
      await sendEmail(email, 'Welcome to HoldKart!',
        `Hi ${name},\n\nWelcome to HoldKart! Shop the best refurbished electronics.\n\nTeam HoldKart`);
    } catch {}
  ...
  export const forgotPassword = async ({ email }) => {
    ...
    try {
      await sendEmail(email, 'HoldKart — Reset Your Password',
        `Click the link to reset your password (valid 15 min):\n\n${link}\n\nTeam HoldKart`);
    } catch {}
